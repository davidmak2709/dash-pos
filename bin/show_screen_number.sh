...
function gen_screen_payment_pending(){
    ...
    /usr/bin/qrencode -s 9 -l M -m 0 -o $OUTDIR/qr.png 'dash:'$addinput'?amount='$amount'&label='$QR_PAYMENT_LABEL'&IS=1'
    ...
    /usr/bin/convert $IMG_BG_PAYMENT_SCREEN \
        -fill "rgb(28,117,188)" \
        -pointsize 30 -draw "text 23,80 'SEND $amount'" \
        -pointsize 45 -draw "text 23,165  'WITH                   TO'
                             text 23,705  'TAKE BEVERAGE'
